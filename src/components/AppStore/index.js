...
class AppStore extends Component {
  constructor(props) {
    ...
    this.state = {
      activeTab: 'SOCIAL',
      searchQuery: '',
      tabsArrayList: tabsList,
      appsArrayList: appsList,
    }
  }
  ...
  render() {
    const {tabsArrayList, appsArrayList} = this.state
    const {activeTab, searchQuery} = this.state

    const filteredApps = appsArrayList.filter(
      app =>
        (app.category === activeTab || activeTab === 'All') &&
    ...
    return (
      <div className="app-store-container">
        ...
        {/* Tabs */}
        <ul className="tabs">
          {tabsArrayList.map(tab => (
            <TabItem
              key={tab.tabId} // Unique key for each tab
              tabId={tab.tabId}
        ...
        <ul className="apps-list">
          {filteredApps.map(app => (
            <AppItem key={app.appId} app={app} /> // Unique key for each app item
          ))}
        </ul>
      </div>
    )
